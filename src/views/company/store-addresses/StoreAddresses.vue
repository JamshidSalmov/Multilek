<template>
  <b-row>
    <b-col cols="12" class="addBtn mb-2" style="" v-if="!busy">
      <router-link :to="{name: 'store-address-add'}">
    ...
    <b-col cols="12">
      ...
      >
        ...
        <template #cell(actions)="data">
          <router-link
            :to="{name: 'store-address-view', params: {id: data.item.id}}"
          >
          ...
          <router-link
            :to="{name: 'store-address-edit', params: {id: data.item.id}}"
          >
...
export default {
  ...
  methods: {
    ...
    async delete(id) {
      await axios
        .delete(`${this.$baseUrl}/map/detail/${id}/`)
        .then(() => {
          this.fetchData();
          this.$toast({
