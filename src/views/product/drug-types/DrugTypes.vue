<template>
  <b-row>
    <b-col cols="12" class="addBtn mb-2" style="" v-if="!busy">
      <router-link :to="{name: 'drug-type-add'}">
    ...
    <b-col cols="12">
      ...
      >
        ...
        <template #cell(actions)="data">
          <div class="boxx">
              <router-link
                :to="{name: 'drug-type-edit', params: {id: data.item.id}}"
              >
                <div class="btnn">
  ...
  methods: {
    ...
    async fetchData() {
      ...
      this.items = await axios
        .get(`${this.$baseUrl}/product/type`, params)
        .then((res) => {
    ...
    async delete(id) {
      await axios
        .delete(`${this.$baseUrl}/product/type/${id}/`)
        .then(() => {
          this.fetchData();
          this.$toast({
