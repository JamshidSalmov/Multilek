<template>
  <b-row>
    <b-col cols="12" class="addBtn mb-2" style="" v-if="!busy">
      <router-link :to="{name: 'courier-add'}">
    ...
    <b-col cols="12">
      ...
      >
        ...
        <template #cell(actions)="data">
          <router-link
            :to="{name: 'courier-edit', params: {id: data.item.id}}"
          >
...
export default {
  ...
  methods: {
    ...
    async fetchData() {
      await axios.get(`${this.$baseUrl}/dashboard/courier-dashboard/`).then((res) => {
        this.items = res.data;
    ...
    async delete(id) {
      await axios
        .delete(`${this.$baseUrl}/dashboard/courier-dashboard/${id}/`)
        .then(() => {
          this.fetchData();
          this.$toast({
